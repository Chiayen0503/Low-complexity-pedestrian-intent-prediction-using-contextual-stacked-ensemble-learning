...
class GRU(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes, device):
        ...
        self.gru = nn.GRU(input_size, hidden_size, num_layers, batch_first=True).to(device)#batch, seq, input_size
        self.device = device
    def forward(self, x):
        device = self.device
        h0 = torch.zeros(self.num_layers, x.shape[0], self.hidden_size).to(device)#x.shape[0]: batch size
        out, last = self.gru(x, h0)#take two inputs if num_layers==2, out = [N, seq, hn] #all hidden state
        last = torch.squeeze(last)

        return out, last
    
    
class GRUattention(nn.Module):#batch, frame, 512
    def __init__(self, input_size, hidden_size, num_layers, num_classes, device):#batch, frame, post-proccessed seq
        super(GRUattention, self).__init__()
        self.gru  = GRU(input_size, hidden_size, num_layers, num_classes, device)#batch, hidden
        self.attention = Attention(device, method=AttentionModel.GENERAL, hidden_size=hidden_size).to(device)
        self.fc = nn.Linear(hidden_size, num_classes).to(device)

    def forward(self, surround):
        surround_g, last_g = self.gru(surround)
        attn_hidden, attn_weights = self.attention(surround_g, last_g)
        output = self.fc(attn_hidden)
...
def train_gru_att(train_loader, val_loader, save_model_folder_acc, save_model_folder_f1, loss_name):
    epochs = 100 
    input_size = 512
