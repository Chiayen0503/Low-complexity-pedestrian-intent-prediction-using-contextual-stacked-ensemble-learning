...
class CNN_ForecastNet(torch.nn.Module):
    def __init__(self):
        super(CNN_ForecastNet,self).__init__()
        self.conv1d_1 = nn.Conv1d(5,15,kernel_size=3)
        self.conv1d_2 = nn.Conv1d(15,10,kernel_size=1)
        self.relu_1 = nn.ReLU(inplace=True)
        self.dropout_1 = nn.Dropout(p=0.5)
        self.dropout_2 = nn.Dropout(p=0.5)
        self.fc1=nn.Linear(140,1) #dim 1 * dim 2
        
    def forward(self,x):
        x = torch.permute(x, (0, 2, 1)) #batch_size, 300, 3 -> batch_size, 3, 300
        x = self.conv1d_1(x)
        x = self.relu_1(x)
        x = self.dropout_1(x)
        x = self.conv1d_2(x)
        x = self.relu_1(x)
        x = self.dropout_2(x)
        x = x.view(x.size(0), -1) #flatton the the rest of dims but keep the batch size
        x = self.fc1(x)
        return x       
...
def train_1dcnn_addfeature(train_loader, val_loader, save_model_folder_acc, save_model_folder_f1, loss_name, alpha):
    epochs = 200
